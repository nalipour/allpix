...
G4VPhysicalVolume * AllPixDetectorConstruction::Construct()
{
	...
	invisibleVisAtt->SetForceSolid(true);
...
void AllPixDetectorConstruction::BuildPixelDevices(map<int, AllPixGeoDsc *> geoMap) {
	...
	for( ; detItr != m_detId.end() ; detItr++)
	{
     ...
     /*------------------------------------ Solid definitions ----------------------------------------*/
		...
		G4Box * Box_GuardRings_Ext = new G4Box(GuardRingsExtName.second,
		...
		if(geoMap[*detItr]->GetHalfPCBZ()!=0){
			PCB_box = new G4Box(PCBName.second,
		...
		G4double wrapperDX = 2*geoMap[*detItr]->GetHalfWrapperDX();
		G4double wrapperDY = 2*geoMap[*detItr]->GetHalfWrapperDY();
		G4double wrapperDZ = 2*geoMap[*detItr]->GetHalfWrapperDZ();
		...
		if(m_vectorWrapperEnhancement.find(*detItr) != m_vectorWrapperEnhancement.end()){
			wrapperDX += m_vectorWrapperEnhancement[*detItr].x()/2.; // half
			wrapperDY += m_vectorWrapperEnhancement[*detItr].y()/2.;
			wrapperDZ += m_vectorWrapperEnhancement[*detItr].z()/2.;
			// temp test
			wrapperDZ += 10000*um;
		}

		G4cout << "Wrapper Dimensions : "
				<< TString::Format("hX=%3.3f hY=%3.3f hZ=%3.3f",wrapperDX,wrapperDY,wrapperDZ)
		<< G4endl;
		...
		posBumps.setZ(posDevice.z()-geoMap[*detItr]->GetHalfSensorZ()+ geoMap[*detItr]->GetChipZOffset()- bump_height/2);

		posChip.setX(posDevice.x() + geoMap[*detItr]->GetChipXOffset());
		posChip.setY(posDevice.y() + geoMap[*detItr]->GetChipYOffset());
		posChip.setZ(posDevice.z()-geoMap[*detItr]->GetHalfSensorZ()-geoMap[*detItr]->GetHalfChipZ()+ geoMap[*detItr]->GetChipZOffset()-bump_height);
		...
		m_Box_phys[(*detItr)] = new G4PVPlacement(0,
				posDevice,
				m_Box_log[(*detItr)],
